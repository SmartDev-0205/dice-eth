...
const diceLoading = require("assets/images/rolling-dice-gif-3.gif");

const diceAbi = require("assets/json/Dice.json");
...
class Dice extends React.Component<Props, DiceState> {
    ...
    private playDice = async () =>  {
        const account = this.props.web3.account;
        const web3 = this.props.web3.web3;
        if (!web3) return;
        const contract = new web3.eth.Contract(diceAbi, CONTRACT_ADDRESS);
        console.log("stress contract",contract);
        const diceplay = contract.methods.casinoPlay;
        console.log("game vlaue-----",this.props.dice.value * 10 **  9,this.props.dice.num)
        let tx = diceplay(this.props.dice.num)
                .send({from: account, value: this.props.dice.value * 10 **  9, gas: 120000})
        return tx;
    };
    ...
    onPlaceBet = async() => {
        ...
        this.playDice().then(async ()=>{
            ...
            if (info.sound) {
                setTimeout(() => (result<=this.props.dice.num ? playFromBegin(sounds.win) : playFromBegin(sounds.lose)), 500);
            }
    
            let resultStr = 'Congratulation.\n You Win.';
            if (result>this.props.dice.num)
                resultStr = 'We are sorry.\nYou loss.';
            w.toastr.info(resultStr);
            console.log("finshed--------------------------");
